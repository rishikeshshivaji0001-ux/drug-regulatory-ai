...
st.set_page_config(page_title="Drug Regulatory AI", layout="wide")

st.title("💊 Drug Regulatory Classification System")
st.markdown("### AI-Powered Regulatory Risk Assessment Tool")
...
Side_Effect_Severity_Score = st.sidebar.slider("Side Effect Severity", 0.0,10.0,5.0)
Abuse_Potential_Score = st.sidebar.slider("Abuse Potential", 0.0,10.0,3.0)
...
Export_Percentage = st.sidebar.number_input("Export %", value=30.0)
Online_Sales_Percentage = st.sidebar.number_input("Online Sales %", value=20.0)
Brand_Reputation_Score = st.sidebar.slider("Brand Reputation", 0.0,10.0,8.0)
...
if st.sidebar.button("🚀 Run Prediction"):
    ...
    if prediction[0] == 1:
        st.error("⚠️ REGULATED DRUG")
    else:
        st.success("✅ NON-REGULATED DRUG")
